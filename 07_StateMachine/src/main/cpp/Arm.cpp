...
Arm::Arm() {
	...
	armEnc = new Encoder(PORTS::ARM_ENCODER_A, PORTS::ARM_ENCODER_B, true, Encoder::EncodingType::k4X);
	armEnc->Reset();
	liftEnc = new Encoder(PORTS::LIFT_ENCODER_A, PORTS::LIFT_ENCODER_B, true, Encoder::EncodingType::k4X);
	liftEnc->Reset();
...
bool Arm::Zeroed(){
	return armZeroed && liftZeroed;
}

/**
 * Sets the arm and lift using controller input
 */
void Arm::ControllerMove(Joystick *rightJoy, Joystick *buttonBoard, bool manual) {
	...
	SmartDashboard::PutNumber("arm top limit", armTopLimit->Get());
	...
	if (armTopLimit->Get() && !armZeroed) { // If limit is not pressed and we have not zeroed yet
		...
		if (buttonBoard->GetRawButton(BUTTONS::ARM_UP) && manual)
			ArmSet(arm_speed);
		else if (buttonBoard->GetRawButton(BUTTONS::ARM_DOWN) && manual)
	...
	if (liftBottomLimit->Get() && !liftZeroed) { // If limit is not pressed and we have not zeroed yet
		...
		if(!liftZeroed)
			liftZeroed = true;
		if (buttonBoard->GetRawButton(BUTTONS::LIFT_UP) && manual)
			LiftSet(lift_speed);
		else if (buttonBoard->GetRawButton(BUTTONS::LIFT_DOWN) && manual)
...
bool Arm::LiftPID(double goal) {
	...
	// if (speed < 0 && speed > -0.25) speed = -0.25;
	SmartDashboard::PutNumber("lift PID speed", speed);
	LiftSet(speed);
	return false;
}
